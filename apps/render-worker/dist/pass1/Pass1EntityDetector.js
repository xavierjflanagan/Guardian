...
class Pass1EntityDetector {
    ...
    async callAIForEntityDetection(input) {
        ...
        if (useMinimalPrompt) {
            ...
            prompt = (0, pass1_prompts_minimal_test_1.generateMinimalListPrompt)(input);
            systemMessage = pass1_prompts_minimal_test_1.MINIMAL_SYSTEM_MESSAGE;
        }
        else {
            prompt = (0, pass1_prompts_1.generatePass1ClassificationPrompt)(input, this.config.model);
            systemMessage = pass1_prompts_1.PASS1_SYSTEM_MESSAGE;
        }
        ...
        const response = await (0, retry_1.retryOpenAI)(async () => {
        ...
        if (useMinimalPrompt) {
            ...
            if (rawResult.entities && Array.isArray(rawResult.entities)) {
                this.logger.debug('Minimal prompt extracted entities', {
                    shell_file_id: input.shell_file_id,
                    entity_texts: rawResult.entities.map((e) => e.text).join(' | '),
                });
            }
     ...
     * @returns Processing result with database records
     */
    async processDocumentOCROnly(input) {
        const startTime = Date.now();
        try {
            ...
            // Step 8: Build all database records (7 tables)
            const databaseRecords = (0, pass1_database_builder_1.buildPass1DatabaseRecordsOCROnly)(input, aiResponse, sessionMetadata, entityRecords, processingTimeMs);
            // Step 9: Return success result
            return {
                success: true,
                processing_session_id: input.processing_session_id,
     ...
     * @returns Parsed AI response
     */
    async callAIForEntityDetectionOCROnly(input) {
        const startTime = Date.now();
        const prompt = (0, pass1_prompts_1.generatePass1ClassificationPromptOCROnly)(input.enhanced_ocr_text, this.config.model);
        const systemMessage = pass1_prompts_1.PASS1_SYSTEM_MESSAGE_OCR_ONLY;
     ...
     */
    async getAllDatabaseRecordsOCROnly(input) {
        const sessionMetadata = {
            shell_file_id: input.shell_file_id,
            patient_id: input.patient_id,
            processing_session_id: input.processing_session_id,
        ...
        // Extract timing from AI response metadata
        const processingTimeMs = aiResponse.processing_metadata.processing_time_seconds * 1000;
        return (0, pass1_database_builder_1.buildPass1DatabaseRecordsOCROnly)(input, aiResponse, sessionMetadata, entityRecords, processingTimeMs);
    }
}
