...
exports.selectCodeCandidates = selectCodeCandidates;
exports.combineAndRankCandidates = combineAndRankCandidates;
exports.applyEntityTypeFiltering = applyEntityTypeFiltering;
exports.processCodeCandidates = processCodeCandidates;
const config_1 = require("./config");
const logger_1 = require("../utils/logger");
/**
 * Select optimal code candidates using hybrid confidence-based approach
 */
function selectCodeCandidates(rawCandidates, config = config_1.PASS15_CONFIG.candidate_selection) {
    if (rawCandidates.length === 0) {
        logger_1.logger.debug('No candidates to select from');
        return [];
    }
    // Step 1: Filter out low similarity candidates
    const filtered = rawCandidates.filter(candidate => candidate.similarity_score >= config.MIN_SIMILARITY);
    if (filtered.length === 0) {
        logger_1.logger.debug('All candidates below minimum similarity threshold', {
    ...
    const highConfidence = filtered.filter(candidate => candidate.similarity_score >= config.AUTO_INCLUDE_THRESHOLD);
    // Step 3: Fill to target of 10 candidates
    const remaining = filtered.filter(candidate => candidate.similarity_score < config.AUTO_INCLUDE_THRESHOLD);
    ...
    logger_1.logger.debug('Candidate selection completed', {
 ...
 */
function combineAndRankCandidates(universalCandidates, regionalCandidates) {
    ...
    logger_1.logger.debug('Candidate combination completed', {
 ...
 */
function applyEntityTypeFiltering(candidates, entitySubtype) {
 ...
 */
function processCodeCandidates(universalCandidates, regionalCandidates, entitySubtype) {
    ...
    logger_1.logger.debug('Candidate processing pipeline completed', {
