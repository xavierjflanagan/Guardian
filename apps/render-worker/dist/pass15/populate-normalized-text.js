 ...
 */
async function fetchCodesToNormalize(limit) {
 ...
 */
async function processBatch(codes, stats) {
    ...
    for (let i = 0; i < codes.length; i += BATCH_SIZE) {
        ...
        const updates = batch.map(code => {
            try {
                const normalizedText = (0, normalization_1.normalizeText)(code.display_name, code.search_text, code.entity_type);
                stats.processed++;
                return {
                    id: code.id,
 ...
 */
async function displaySampleResults(limit = 20) {
    console.log('\n' + '='.repeat(80));
 ...
 */
async function populateNormalizedText(dryRun = false) {
    const stats = {
        total: 0,
        processed: 0,
        updated: 0,
...
//# sourceMappingURL=populate-normalized-text.js.map