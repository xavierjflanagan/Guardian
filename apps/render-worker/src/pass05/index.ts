...
export async function runPass05(input: Pass05Input): Promise<Pass05Output> {
  ...
  try {
    ...
    // Strategy A (V11): Encounters written to DB during progressive processing, not returned here
    const manifest: ShellFileManifest = {
      shellFileId: input.shellFileId,
      patientId: input.patientId,
      totalPages: input.pageCount,
      ocrAverageConfidence: calculateAverageConfidence(input.ocrOutput),
      encounters: encounterResult.encounters!,  // Always [] for Strategy A
      page_assignments: encounterResult.page_assignments,  // Always [] for Strategy A
      batching: null  // Phase 1: always null
    };

    // Migration 61: Removed dead code (lines 113-178)
    // - HOTFIX 2A: ai_processing_sessions insert (never executed - Strategy A returns encounters:[])
    // - HOTFIX 2A: pass05_encounter_metrics insert (never executed - metrics created by RPC)
    // See: migration_history/2025-11-21_61_remove_pass05_dead_code.md
