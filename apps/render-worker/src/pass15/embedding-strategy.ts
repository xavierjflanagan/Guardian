...
import { ENTITY_TYPE_STRATEGIES } from './config';
...
export function getEmbeddingText(entity: Pass1Entity): string {
  ...
  if (ENTITY_TYPE_STRATEGIES.VITAL_TYPES.includes(subtype as any)) {
  ...
  // Procedures: Use expanded descriptions when available
  if (ENTITY_TYPE_STRATEGIES.PROCEDURE_TYPES.includes(subtype as any)) {
    if (entity.ai_visual_interpretation && 
        entity.ai_visual_interpretation.length > entity.original_text.length) {
      return entity.ai_visual_interpretation;
    }
  ...
  if (ENTITY_TYPE_STRATEGIES.IDENTIFIER_TYPES.includes(subtype as any)) {
    return entity.original_text;
  }
