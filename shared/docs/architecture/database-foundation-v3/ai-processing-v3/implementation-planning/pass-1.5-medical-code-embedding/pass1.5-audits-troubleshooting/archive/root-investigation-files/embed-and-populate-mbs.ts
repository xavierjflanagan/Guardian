/**
 * MBS Embedding and Population Script
 * 
 * Purpose: Generate embeddings and populate database with MBS procedure codes
 * Based on: embed-and-populate-pbs.ts (successful PBS implementation)
 */
...
interface EmbeddedMBSCode extends MBSCode {
  embedding: number[];
}
 ...
 */
async function generateEmbeddingBatch(codes: MBSCode[]): Promise<EmbeddedMBSCode[]> {
 ...
 */
function toDBFormat(code: EmbeddedMBSCode, batchId: string) {
  // Determine complexity level based on group
  const complexityLevel = determineComplexityLevel(code.region_specific_data?.group);
  ...
  return {
    ...
    superseded_by: null,
    active: true,
    authority_required: false, // MBS procedures don't require authority
    mbs_complexity_level: complexityLevel,
    // Migration 28 fields with MBS-appropriate defaults
    clinical_specificity: 'moderately_specific', // MBS procedures more specific than PBS
    typical_setting: 'any', // MBS covers all settings (GP, specialist, hospital)
    embedding_batch_id: batchId,
  };
}

/**
 * Determine complexity level based on MBS group
 */
function determineComplexityLevel(group: string): string {
  if (!group) return 'basic';
  
  // Group mapping based on MBS structure
  const firstChar = group.charAt(0);
  
  switch (firstChar) {
    case 'A': return 'basic'; // Professional consultations
    case 'T': return 'complex'; // Operations and procedures
    case 'P': return 'complex'; // Pathology
    case 'I': return 'intermediate'; // Diagnostic imaging
    case 'D': return 'intermediate'; // Dental
 ...
 */
async function main() {
  console.log('🚀 Starting MBS Direct Embedding and Population');
  ...
  // 2. Load MBS codes
  const mbsPath = '/Users/xflanagan/Documents/GitHub/Guardian-Cursor/data/medical-codes/mbs/processed/mbs_codes.json';
  const codes: MBSCode[] = await fs.readJson(mbsPath);
  console.log(`📂 Loaded ${codes.length} MBS procedure codes`);
  ...
  for (let i = 0; i < codes.length; i += chunkSize) {
    ...
    try {
      ...
      // Convert to database format
      const dbRecords = embeddedCodes.map(code => toDBFormat(code, batchRecord.id));
  ...
  console.log(`🔍 Database validation: ${validation?.count || 'unknown'} MBS codes in database`);
  console.log(`💰 Estimated cost: ~$0.03 USD (${totalProcessed} codes × ~0.000005 per code)`);
}

// Validate environment
if (!process.env.OPENAI_API_KEY || !process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
  console.error('❌ Missing required environment variables');
  console.error('Required: OPENAI_API_KEY, NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY');
  process.exit(1);
}
