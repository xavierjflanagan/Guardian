...
29th Nov batch fail amnagement:

 1. Partial Batch Failure - Option A Design

  You're right - we shouldn't proceed to Pass 2 with incomplete data. Option A (retry failed batches while
  successful ones wait) is the correct approach. Here's the elegant design:
  ...
  async function processEncounterWithRetry(
    ...
    const batchStates: BatchResult[] = batches.map((batch, i) => ({
      batchIndex: i,
    ...
    while (true) {
      // Find batches that need processing (pending or failed with retries remaining)
      ...
      // Process failed/pending batches in parallel
      ...
      for (const settledResult of results) {
        if (settledResult.status === 'fulfilled') {
          ...
          batchStates[batchIndex].result = result;
        } else {
          // Find which batch failed (from the error context)
    ...
    if (!allSucceeded) {
      // After max retries, some batches still failed
    ...
    // All succeeded - merge results in order
    return mergeOrderedBatchResults(batchStates);
  }

  Key design points:

  1. Successful batches wait in memory - their results are stored in batchState.result
  2. Only failed batches retry - no wasted API calls on already-succeeded batches
  3. Ordered merge at the end - results combined by batchIndex to maintain page order
  4. Exponential backoff between retry rounds - respects rate limits
  5. Clear failure after max retries - if batches still fail after 3 attempts, the entire encounter fails
  (not partial)

  Status flow:
  - All batches succeed -> pass1_complete
  - Some batches fail after max retries -> Job fails, stays at pass05_complete, can be retried later

  This is cleaner than pass1_partial status - we either fully complete or we don't proceed.